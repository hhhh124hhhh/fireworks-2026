...
# Content Hotspot Collector v4.0
# 多平台热点收集脚本（带 AI 摘要 - 使用 coding-agent）
# 用于自媒体创作者的内容灵感发现
...
AI_SUMMARY_ENABLED=true  # 使用 coding-agent
...
TEMP_FILE="/tmp/hotspots-v4-$DATETIME.txt"
...
echo -e "${GREEN}=== Content Hotspot Collector v4.0 (AI 摘要 - coding-agent）===${NC}"
echo "开始收集真实热点..."
echo ""
...
# ============================================
# AI 摘要函数（使用 coding-agent）
# ============================================
get_ai_summary() {
    local url="$1"
    local summary=""

    if [ ! "$AI_SUMMARY_ENABLED" = "true" ]; then
        echo ""
        return
    fi
    ...
    # 获取 URL 内容（限制 5000 字符）
    local content=$(curl -s --max-time 15 "$url" 2>/dev/null | head -c 5000 || echo "")
    ...
    # 清理 HTML 标签（简单清理）
    content=$(echo "$content" | sed 's/<script[^>]*>.*<\/script>//g' | sed 's/<style[^>]*>.*<\/style>//g' | sed 's/<[^>]*>//g' | sed 's/&nbsp;/ /g')

    # 限制长度（2000 字符）
    content=$(echo "$content" | head -c 2000)

    # 生成提示词
    local prompt="请用 2-3 句话总结以下网页内容（中文），这个网页的内容来自：$url

网页内容：
$content

请直接输出摘要，不要说'以下是'、'摘要是'等前缀。"

    # 使用 Python 调用 sessions_spawn
    local session_id="hotspot-summary-$(date +%s%N | head -c 6)"

    # 临时文件用于存储结果
    local result_file="/tmp/hotspot-summary-$session_id.txt"

    # 启动 coding-agent 会话
    echo "$prompt" > "$result_file.prompt"
    
    # 使用 openclaw 命令发送消息到 coding-agent
    # 注意：这里需要调用 OpenClaw 的 API，可能需要等待实现
    # 暂时返回简单摘要
    
    echo "网站内容摘要：访问链接查看详情"
}
...
" 2>/dev/null || echo "")
            ...
            if [ -n "$STORY_INFO" ] && [ "$STORY_INFO" != "" ]; then
                ...
                # 写入临时文件
                echo "HN|$TITLE|$SCORE|$URL|$TRANSLATED_TITLE|$SUMMARY" >> "$TEMP_FILE"
                COUNT=$((COUNT + 1))
            fi
        fi
    done
...
**总计热点**: $TOTAL 个
**AI 摘要**: ${AI_SUMMARY_ENABLED}（coding-agent）
...
# 添加热点到报告（所有热点）
INDEX=1
while IFS='|' read -r SOURCE TITLE SCORE URL TRANSLATED_TITLE SUMMARY; do
    if [ -z "$SOURCE" ]; then
        continue
    fi
    ...
    if [ -n "$SUMMARY" ] && [ "$SUMMARY" != "" ]; then
        echo "- **AI 摘要**: $SUMMARY" >> "$REPORT_FILE"
    fi
...
if [ -n "$HN_HOTSPOTS" ]; then
    echo "$HN_HOTSPOTS" | while IFS='|' read -r SOURCE TITLE SCORE URL TRANSLATED_TITLE SUMMARY; do
        if [ -z "$SOURCE" ]; then
            continue
        fi
...
echo "  - AI 摘要: ${AI_SUMMARY_ENABLED}（coding-agent）"
echo ""
