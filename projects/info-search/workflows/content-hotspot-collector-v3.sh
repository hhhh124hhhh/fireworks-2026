...
# Content Hotspot Collector v3.0
# 多平台热点收集脚本（带内容摘要 - 混合方案）
# 用于自媒体创作者的内容灵感发现
...
SUMMARY_LENGTH=500  # URL 预览长度（字符）
AI_SUMMARY_ENABLED=false  # AI 摘要（需要配置 API）

# AI 摘要 API 配置（可选）
# AI_SUMMARY_API_KEY=""  # OpenAI API Key
# AI_SUMMARY_MODEL="gpt-4-mini"  # 模型名称
...
echo -e "${GREEN}=== Content Hotspot Collector v3.0 (带内容摘要）===${NC}"
echo "开始收集真实热点..."
echo ""
...
get_url_preview() {
    ...
    if [ "$SUMMARY_ENABLED" = "true" ] && [ -n "$url" ] && [ "$url" != "null" ]; then
        # 使用 web_fetch 获取内容
        local content=$(curl -s --max-time 10 "$url" 2>/dev/null | head -c $SUMMARY_LENGTH || echo "")

        # 清理内容（移除 HTML 标签）
        preview=$(echo "$content" | sed 's/<[^>]*>//g' | sed 's/&nbsp;/ /g' | tr -d '\n' | sed 's/  */ /g' | sed 's/^ *//;s/ *$//')

        # 如果内容为空，尝试其他方法
        if [ -z "$preview" ]; then
            preview="无法预览内容，请访问链接查看详情"
        fi
    fi
...
" 2>/dev/null || echo "")
            ...
            if [ -n "$STORY_INFO" ] && [ "$STORY_INFO" != "" ]; then
                ...
                # 获取 URL 内容预览
                echo -n "  [${GREEN}+${NC}] 获取内容预览: $TRANSLATED_TITLE... "
                PREVIEW=$(get_url_preview "$URL")
                echo -e "${GREEN}完成${NC}"

                # 如果启用了 AI 摘要，尝试生成
                SUMMARY=""
                if [ "$AI_SUMMARY_ENABLED" = "true" ] && [ -n "$PREVIEW" ] && [ "$PREVIEW" != "无法预览内容，请访问链接查看详情" ]; then
                    echo -n "  [${GREEN}+${NC}] 生成 AI 摘要... "
                    SUMMARY=$(get_ai_summary "$PREVIEW")
                    if [ -n "$SUMMARY" ]; then
                        echo -e "${GREEN}完成${NC}"
                    else
                        echo -e "${YELLOW}失败（使用预览）${NC}"
                    fi
                fi

                # 写入临时文件（使用特殊分隔符避免问题）
                echo "HN|$TITLE|$SCORE|$URL|$TRANSLATED_TITLE|$PREVIEW|$SUMMARY" >> "$TEMP_FILE"
                COUNT=$((COUNT + 1))
            fi
        fi
    done
...
# 添加热点到报告（所有热点）
INDEX=1
while IFS='|' read -r SOURCE TITLE SCORE URL TRANSLATED_TITLE PREVIEW SUMMARY; do
    if [ -z "$SOURCE" ]; then
        continue
    fi
    ...
    # 如果有 AI 摘要，使用 AI 摘要；否则使用 URL 预览
    if [ -n "$SUMMARY" ] && [ "$SUMMARY" != "" ]; then
        echo "- **摘要**: $SUMMARY" >> "$REPORT_FILE"
    elif [ -n "$PREVIEW" ] && [ "$PREVIEW" != "" ] && [ "$PREVIEW" != " " ]; then
        # 限制预览长度
        PREVIEW_DISPLAY=$(echo "$PREVIEW" | cut -c1-200)
        echo "- **内容预览**: $PREVIEW_DISPLAY..." >> "$REPORT_FILE"
    fi
...
if [ -n "$HN_HOTSPOTS" ]; then
    echo "$HN_HOTSPOTS" | while IFS='|' read -r SOURCE TITLE SCORE URL TRANSLATED_TITLE PREVIEW SUMMARY; do
        if [ -z "$SOURCE" ]; then
            continue
        fi
...
echo "  - 内容摘要: ${SUMMARY_ENABLED}（URL 预览）"
