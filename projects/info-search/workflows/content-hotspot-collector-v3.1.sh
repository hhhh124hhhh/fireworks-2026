...
# Content Hotspot Collector v3.1
# 多平台热点收集脚本（带内容摘要 - 改进版）
# 用于自媒体创作者的内容灵感发现
...
echo -e "${GREEN}=== Content Hotspot Collector v3.1 (改进版内容摘要）===${NC}"
echo "开始收集真实热点..."
echo ""
...
get_content_summary() {
    ...
    if command -v web_fetch &> /dev/null; then
        ...
        if [ -n "$content" ] && [ ${#content} -gt 100 ]; then
            # 提取前 300 字符
            summary=$(echo "$content" | head -c 300 | sed 's/[[:space:]]\+/ /g' | sed 's/^ //;s/ $//')
        fi
    fi

    # 如果 web_fetch 失败，尝试直接获取并清理
    if [ -z "$summary" ]; then
        ...
        if [ -n "$html" ]; then
            ...
            html=$(echo "$html" | sed 's/<script[^>]*>.*<\/script>//g' | sed 's/<style[^>]*>.*<\/style>//g')
            # 移除所有 HTML 标签
            text=$(echo "$html" | sed 's/<[^>]*>//g' | sed 's/&nbsp;/ /g')
            # 提取前 200 字符
            summary=$(echo "$text" | tr -d '\n' | sed 's/[[:space:]]\+/ /g' | head -c 200 | sed 's/^ //;s/ $//')
        fi
    fi

    # 如果仍然为空，返回默认消息
    if [ -z "$summary" ] || [ ${#summary} -lt 20 ]; then
        summary="建议直接访问链接查看详情"
    fi
...
" 2>/dev/null || echo "")
            ...
            if [ -n "$STORY_INFO" ] && [ "$STORY_INFO" != "" ]; then
                ...
                SUMMARY=$(get_content_summary "$URL")
                echo -e "${GREEN}完成${NC}"

                # 如果启用了 AI 摘要，尝试生成
                AI_SUMMARY=""
                if [ "$AI_SUMMARY_ENABLED" = "true" ] && [ -n "$SUMMARY" ] && [ "$SUMMARY" != "建议直接访问链接查看详情" ]; then
...
while IFS='|' read -r SOURCE TITLE SCORE URL TRANSLATED_TITLE SUMMARY AI_SUMMARY; do
    ...
    if [ -n "$AI_SUMMARY" ] && [ "$AI_SUMMARY" != "" ]; then
        echo "- **摘要**: $AI_SUMMARY" >> "$REPORT_FILE"
