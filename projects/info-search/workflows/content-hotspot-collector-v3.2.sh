...
# Content Hotspot Collector v3.2
# 多平台热点收集脚本（带内容摘要 - 使用 jina.ai）
# 用于自媒体创作者的内容灵感发现
...
echo -e "${GREEN}=== Content Hotspot Collector v3.2 (jina.ai 内容摘要）===${NC}"
echo "开始收集真实热点..."
echo ""
...
get_url_summary() {
    ...
    if [ -n "$content" ] && [ ${#content} -gt 50 ]; then
        # 移除多余的空白
        summary=$(echo "$content" | tr -s '[:space:]' ' ' | sed 's/^ //;s/ $//')

        # 限制长度
        summary=$(echo "$summary" | cut -c1-$SUMMARY_LENGTH)
...
echo ""
echo -e "${BLUE}提示:${NC}"
echo "  - 中文翻译: ${TRANSLATED_ENABLED}"
echo "  - 内容摘要: ${SUMMARY_ENABLED}（jina.ai）"
