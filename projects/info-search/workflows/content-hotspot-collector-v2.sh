...
# Content Hotspot Collector v2.0
# 多平台热点收集脚本（带中文翻译）
# 用于自媒体创作者的内容灵感发现
...
TRANSLATE_ENABLED=true  # 设置为 false 禁用翻译
...
TEMP_FILE="/tmp/hotspots-v2-$DATETIME.txt"
...
echo -e "${GREEN}=== Content Hotspot Collector v2.0 (带中文翻译）===${NC}"
echo "开始收集真实热点..."
echo ""
...
" 2>/dev/null || echo "")
            ...
            if [ -n "$STORY_INFO" ] && [ "$STORY_INFO" != "" ]; then
                ...
                echo "  ${GREEN}+${NC} $TRANSLATED_TITLE (score: $SCORE)"
                echo "HN|$TITLE|$SCORE|$URL|$TRANSLATED_TITLE" >> "$TEMP_FILE"
                COUNT=$((COUNT + 1))
            fi
        fi
    done
...
# 添加热点到报告（前 10 个）
INDEX=1
head -n 10 "$TEMP_FILE" 2>/dev/null | while read -r HOTSPOT; do
    if [ -z "$HOTSPOT" ]; then
        continue
    fi
    ...
    TRANSLATED_TITLE=$(echo "$HOTSPOT" | cut -d'|' -f5)
