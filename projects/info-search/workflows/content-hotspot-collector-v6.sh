...
# Content Hotspot Collector v6.0
# 多平台热点收集脚本（带 AI 摘要 - coding-agent）
# 用于自媒体创作者的内容灵感发现
...
echo -e "${GREEN}=== Content Hotspot Collector v6.0 (AI 摘要 - coding-agent）===${NC}"
echo "开始收集真实热点..."
echo ""
...
get_ai_summary() {
    ...
    if [ ! "$AI_SUMMARY_ENABLED" = "true" ]; then
        echo '{"summary":"AI 摘要已禁用","content_direction":"N/A","suggestions":[],"target_audience":"N/A","difficulty":"N/A"}'
        return
    fi

    # 调用 Python 助手
    local result=$(python3 "$AI_ASSISTANT_SCRIPT" "$title" "$url" "$score" 2>/dev/null || echo '{"summary":"生成失败","content_direction":"N/A","suggestions":[],"target_audience":"N/A","difficulty":"N/A"}')
...
" 2>/dev/null || echo "")
            ...
            if [ -n "$STORY_INFO" ] && [ "$STORY_INFO" != "" ]; then
                ...
                AI_RESULT=$(get_ai_summary "$TITLE" "$URL" "$SCORE")
                echo -e "${GREEN}完成${NC}"
...
- **AI 摘要**: ${AI_SUMMARY_ENABLED}（coding-agent：摘要 + 热度评估 + 创作建议）

---

**下次更新**: 明天 08:00
**收集工具**: Content Hotspot Collector v6.0
EOF
...
echo "  - 使用 coding-agent 生成高质量的 AI 摘要和创作建议"
echo ""
