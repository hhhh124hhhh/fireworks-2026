...
QUERY_ID=$(echo "$OUTLINE_OUTPUT" | grep -o '"query_id":"[^"]*"' | head -1 | cut -d'"' -f4)
CHAT_ID=$(echo "$OUTLINE_OUTPUT" | grep -o '"chat_id":"[^"]*"' | head -1 | cut -d'"' -f4)
TITLE=$(echo "$OUTLINE_OUTPUT" | grep -o '"title":"[^"]*"' | head -1 | cut -d'"' -f4)

if [ -z "$QUERY_ID" ] || [ -z "$CHAT_ID" ]; then
    echo "   ❌ 大纲生成失败：无法提取 query_id 或 chat_id" | tee -a "$LOG_FILE"
    exit 1
fi
