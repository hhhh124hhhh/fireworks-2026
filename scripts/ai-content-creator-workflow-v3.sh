...
# AI Content Creator 自动化工作流 (改进版 v3)
# 流程：AI 搜索 → PPT 生成
...
# 从输出中提取 query_id 和 chat_id
# 找到最后一个包含 query_id 的非空值
QUERY_ID=""
CHAT_ID=""

# 使用 tac (反向 cat) 来从后往前找
LAST_RESPONSE=$(echo "$OUTLINE_OUTPUT" | grep '"is_end":true' | tail -1)
if [ -n "$LAST_RESPONSE" ]; then
    QUERY_ID=$(echo "$LAST_RESPONSE" | grep -o '"query_id":"[^"]*"' | head -1 | cut -d'"' -f4)
    CHAT_ID=$(echo "$LAST_RESPONSE" | grep -o '"chat_id":"[^"]*"' | head -1 | cut -d'"' -f4)
    TITLE=$(echo "$LAST_RESPONSE" | grep -o '"title":"[^"]*"' | head -1 | cut -d'"' -f4)
fi
...
# 提取 PPT 下载链接
PPT_URL=$(echo "$PPT_RESULT" | grep '"pptx_url"' | head -1 | grep -o '"pptx_url":"[^"]*"' | cut -d'"' -f4)
