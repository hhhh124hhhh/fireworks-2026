...
# 导入配置模块
try:
    from config import get_base_url, get_endpoint
    API_BASE_URL = get_base_url()
    API_ENDPOINTS = {
        'douyin_search': get_endpoint('douyin_search'),
        'xiaohongshu_search': get_endpoint('xiaohongshu_search'),
        'wechat_channels_search': get_endpoint('wechat_channels_search'),
    }
except ImportError:
    print("❌ 未找到 config.py 配置模块")
    print("   请先运行 tools/generate_obfuscated.py 生成配置")
    sys.exit(1)
